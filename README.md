...
3. 원본 가로세로 비율을 보고 정사각형 타일 그리드(열x행)를 자동 계산해서 100x100 규격으로 자릅니다.
   - `TARGET_TILE_COUNT`(기본 12개)를 "대략 몇 조각으로 나눌지" 목표치로 삼아, 원본 비율에 가장
     가까운 열x행 조합을 역산합니다. 조각이 항상 정사각형이 되도록 열 개수를 먼저 정하고, 그 열
     개수로 가로를 정확히 나눈 값을 타일 크기로 삼아 행 개수를 정합니다(비율 왜곡 없음).
   - 값을 늘리면 조각이 많아져 디테일은 살지만 다 이어붙였을 때 메시지가 커지고, 줄이면
     조각(=결과물 크기)이 작아지는 대신 조각 하나하나가 뭉텅해집니다. 원하는 정도로
     `.env`의 `TARGET_TILE_COUNT`를 조정하세요.
   - 계산된 타일 개수가 `MAX_TILES`(기본 200, 텔레그램 한도)를 넘으면 목표 개수를 줄여가며 재계산합니다.
