...
문구를 입력하면 색/폰트를 입혀서 이미지로 만든 뒤, 사진과 똑같은 파이프라인(팩 선택 → 조각 수 →
결제/무료 → 제작)을 그대로 탑니다.
...
2. **색상 스타일 선택** (버튼): 👑 골드 프리미엄 / 💖 네온 핑크 / 🟢 형광 그린 / 🌸 파스텔 드림 / 🌈 레인보우
   — 배경(단색/그라데이션), 글자 색(단색/그라데이션), 테두리, 네온 스타일은 후광(glow) 효과까지 Pillow로 직접 합성합니다.
3. 렌더링된 이미지가 사진을 보낸 것과 동일하게 팩 선택 단계로 이어집니다.

새 프리셋(폰트/색상 조합)을 추가하려면 `bot/text_emoji.py`의 `FONTS`/`STYLES` 딕셔너리에 항목을 추가하고,
필요한 폰트 파일을 `assets/fonts/`에 넣으면 됩니다.
...
봇에게 `/start`를 보내거나 바로 사진을 전송하면 됩니다. `/status`로 오늘 남은 무료 횟수와
내 이모지 팩 목록을 볼 수 있습니다. 첫 실행 시 `data/bot.db`(SQLite)가 자동으로 생성되고,
텔레그램 클라이언트에서 `/`를 입력하면 `/start`, `/text`, `/status` 명령어 목록이 뜹니다
(봇 시작 시 `setMyCommands`로 자동 등록됩니다).
...
포함하지 않았습니다. 실제로 사진을 보내서 동작을 확인해보세요.

## 알아두면 좋은 제약사항

- 텔레그램 커스텀 이모지(정지)는 PNG/WEBP만 지원하며 항상 100x100이어야 합니다. GIF/영상은 지원하지
  않습니다(이 봇은 정지 이미지만 다룹니다).
...
- [x] 글자 이모지화 (`/text`, 폰트/색상 스타일 선택)
- [ ] 텍스트 이모지에 이펙트(등장/반짝임 등) 애니메이션 추가
