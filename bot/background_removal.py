...
정지 이미지는 [rembg](https://github.com/danielgatis/rembg)(U^2-Net 기반 오픈소스
배경 제거 라이브러리)를 그대로 사용하고, 영상/GIF는 프레임 단위로 rembg를 돌린 뒤
알파 채널을 가진 WEBM(VP9)으로 다시 인코딩한다. 이렇게 만들어진 알파 영상은 이후
video_split.py의 crop/scale 파이프라인에 그대로 입력으로 넣을 수 있다.
"""
